...
function initCanvasPanel() {
    ...
    canvasPanel.init();
    document.getElementById('linkMarkPanel').getElementsByTagName('input').item(0).onclick = function() {
        canvasPanel.refreshCanvasPanel(null);
    };
    document.getElementById('linkMarkPanel').getElementsByTagName('input').item(1).onclick = function() {
        markPanel.linkMardPanelHidden();
        canvasPanel.startCanvasShow(document.getElementById('start'));
        canvasPanel.tableStop();
    };
    document.getElementById('gameOver').getElementsByTagName('input').item(0).onclick = function() {
        canvasPanel.startCanvasShow(document.getElementById('start'));
        canvasPanel.model = null;
        canvasPanel.refreshCanvasPanel(imgs);
    };
}
...
MarkPanel.prototype = {
    ...
    refreshButtonInit: function(count) { //刷新按钮初始化参数为能够刷新的次数
        ...
        if (count == null) {
            ...
            this.refreshButton.onclick = function() {
                _thisMarkPanel.CanvasPanel.refreshCanvasPanel(null);
            };
        } else {
    ...
    timesInit: function() {
        ...
        this.timesId = window.setInterval(function() {
            _thisMarkPanel.timesRefresh()
        }, 1000); //一秒更新一次
    },
...
CanvasPanel.prototype = {
    start: function(evt) {
        ...
        if (modelTemp == this.model) { //如果所选择模式为当前模式直接将home隐藏
            ...
            window.setTimeout(function() {
                _thisStart.MarkPanel.linkMardPanelShow();
                _thisStart.tableRun()
            }, 5000);
        } else {
            //开始新的模式
    ...
    newLevel: function() {
        ...
        switch (this.model) {
            case 0:
                ...
                window.setTimeout(function() {
                    _thisNewLevel.startCanvasHidden(document.getElementById('normal'));
                    _thisNewLevel.refreshCanvasPanel(imgs)
                }, 5000);
                window.setTimeout(function() {
                    normalInit()
                }, 10000);
            ...
            case 1:
                ...
                window.setTimeout(function() {
                    _thisNewLevel.startCanvasHidden(document.getElementById('challenge'));
                    _thisNewLevel.refreshCanvasPanel(imgs)
                }, 5000);
                window.setTimeout(function() {
                    challengeInit()
            ...
            case 2:
                ...
                window.setTimeout(function() {
                    _thisNewLevel.startCanvasHidden(document.getElementById('relaxation'));
                    _thisNewLevel.refreshCanvasPanel(imgs)
                }, 5000);
                window.setTimeout(function() {
                    challengeInit()
            ...
            case 3:
                ...
                window.setTimeout(function() {
                    _thisNewLevel.startCanvasHidden(document.getElementById('nothing'));
                    _thisNewLevel.refreshCanvasPanel(imgs)
                }, 5000);
                window.setTimeout(function() {
                    challengeInit()
    ...
    normal: function() {
        ...
        this.startCanvasHidden(document.getElementById('start'));
        window.setTimeout(function() {
            _thisNormal.startCanvasHidden(document.getElementById('normal'))
        }, 5000);
        window.setTimeout(function() {
            normalInit()
        }, 12000);
    ...
    challenge: function() {
        ...
        this.startCanvasHidden(document.getElementById('start'));
        window.setTimeout(function() {
            _thisChallenge.startCanvasHidden(document.getElementById('challenge'))
        }, 5000);
        window.setTimeout(function() {
            challengeInit()
        }, 12000);
    ...
    relaxation: function() {
        ...
        this.startCanvasHidden(document.getElementById('start'));
        window.setTimeout(function() {
            _thisRelaxation.startCanvasHidden(document.getElementById('relaxation'))
        }, 5000);
        window.setTimeout(function() {
            relaxationInit()
        }, 12000);
    ...
    nothing: function() {
        ...
        this.startCanvasHidden(document.getElementById('start'));
        window.setTimeout(function() {
            _thisNothing.startCanvasHidden(document.getElementById('nothing'))
        }, 5000);
        window.setTimeout(function() {
            nothingInit()
        }, 12000);
    ...
    init: function() {
        ...
        for (var i = 0; i < spans.length; i++) { //初始化开始界面各个模式
            spans.item(i).id = i;
            spans.item(i).onclick = function(event) {
                _this.start(event);
            };
            spans.item(i).onmouseover = function() {
                _this.soundPlay('sounds/spread.wav');
            }
        }
        ...
        var k = 0;
        for (i = 1,len = this.trs.length - 1; i < len; i++) { //初始化this.tds
    ...
    tableRun: function() {
        ...
        for (i = 0,len = this.tds.length; i < len; i++) { //初始化tds获取需要重新布局的td
            if (this.tds[i].style.visibility != 'hidden') {
                this.tds[i].onclick = function(event) {
                    _thisTableRun.clickCanvasPanel(event);
                };
            }
        }
    },
    ...
    refreshCanvasPanel: function(Imgs) { //重新布局画
        ...
        if (Imgs == null) { //传递参数为空即重新洗牌,重新计算imgs
            for (i = 0 , len = this.tds.length; i < len; i++) { //初始化tds获取需要重新布局的td
                if (this.tds[i].style.visibility != 'hidden') {
                    ...
                    j++;
                    subImgs[i] = this.tds[i].style.backgroundImage;     
                }        
            }
            // subImgs = [];
        ...
        for (i = 0,len = tdsTemp.length; i < len; i++) {
    ...
    clickCanvasPanel: function(evt) {
        ...
        function checkTdXY() {
        	var i,j,lenI,lenJ;
            for (i = 1,lenI = _this.trs.length - 1; i < lenI; i++) {
                for (j = 1,lenJ = _this.trs[i].length - 1; j < lenJ; j++) {
                                ...
                                if (_this.previewTdX == _this.currentTdX && _this.previewTdY != _this.currentTdY) {
                                    ...
                                    if (result) {
                                        ...
                                        for (var ii = _this.previewTdX - 1; ii >= 0; ii--) {
                                            ...
                                            if (checkP2P(_this.previewTdX, _this.previewTdY, tempI, _this.previewTdY) && 
                                                 checkP2P(_this.currentTdX, _this.currentTdY, tempI, _this.currentTdY)) { 
                                                 //x到x，能联通切y到y，能联通验证x，到y，能否联通
                                                //window.alert(tempI);
                                                // var resultPxToPy = checkP2P(tempI, _this.previewTdY, tempI, _this.currentTdY);
                                                 if (checkP2P(tempI, _this.previewTdY, tempI, _this.currentTdY)) {
                                                    _this.clearImages();
                                    ...
                                    if (result) {
                                        ...
                                        for (var ii = _this.previewTdY - 1; ii >= 0; ii--) {
                                            ...
                                            //window.alert(resultThisToPointx&&resultThatToPointy)
                                            if (checkP2P(_this.previewTdX, _this.previewTdY, _this.previewTdX, tempI) &&
                                                 checkP2P(_this.currentTdX, _this.currentTdY, _this.currentTdX, tempI)) { 
                                                 //x到x，能联通切y到y，能联通验证x，到y，能否联通
                                                // var resultPxToPy = checkP2P(_this.previewTdX, tempI, _this.currentTdX, tempI);
                                                if (checkP2P(_this.previewTdX, tempI, _this.currentTdX, tempI)) {
                                                    _this.clearImages();
                                    ...
                                    if (_this.trs[_this.previewTdX][_this.currentTdY].style.visibility == 'hidden' && 
                                    	checkP2P(_this.previewTdX, _this.previewTdY, _this.previewTdX, _this.currentTdY) && 
                                    	checkP2P(_this.currentTdX, _this.currentTdY, _this.previewTdX, _this.currentTdY)) {
                                        _this.clearImages();
                                    ...
                                    if (_this.trs[_this.currentTdX][_this.previewTdY].style.visibility == 'hidden' && 
                                    	checkP2P(_this.previewTdX, _this.previewTdY, _this.currentTdX, _this.previewTdY) && 
                                    	checkP2P(_this.currentTdX, _this.currentTdY, _this.currentTdX, _this.previewTdY)) {
                                        result = true;
                                        _this.clearImages();
                                    ...
                                    for (var ii = tempPreviewTdX + 1; ii < tempCurrentTdX; ii++) { //循环查找内折现
                                        ...
                                        if (_this.trs[tempI][tempPreviewTdY].style.visibility != 'hidden' && 
                                         	_this.trs[tempI][tempCurrentTdY].style.visibility != 'hidden') break;
                                        //window.alert(ii);
                                    ...
                                    for (var jj = tempPreviewTdY + 1; jj < tempCurrentTdY; jj++) { //循环查找内折现
                                        ...
                                        // if (_this.trs[tempPreviewTdX][tempJ].style.visibility != 'hidden' && 
                                        // 	_this.trs[tempCurrentTdX][tempJ].style.visibility != 'hidden') break;
                                        //window.alert(jj);
                                        // resultThisToTempX = checkP2P(tempPreviewTdX, tempPreviewTdY, tempPreviewTdX, tempJ);
                                        if (checkP2P(tempPreviewTdX, tempPreviewTdY, tempPreviewTdX, tempJ) &&
                                        	) {
                                            //this-x能联通
                                            // resultThatTopTempY = checkP2P(tempCurrentTdX, tempCurrentTdY, tempCurrentTdX, tempJ);
                                            if (checkP2P(tempCurrentTdX, tempCurrentTdY, tempCurrentTdX, tempJ)) {
                                                //that-y能联通
                                    ...
                                    //if (checkXtoThat(_this,tempCurrentTdX,tempCurrentTdY,tempPreviewTdX,tempPreviewTdY)){
                                    //	return;
                                    //}
